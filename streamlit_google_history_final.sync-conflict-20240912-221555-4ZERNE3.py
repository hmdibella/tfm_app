...
def main():
    ...
    for message in st.session_state.messages:
        avatar = st.image(avatar_assistant, width=20) if message["role"] == "Semaforín" else st.image(avatar_user, width=20)
        with st.chat_message(message["role"], avatar=avatar):
            st.markdown(message["content"])
            
    # HDB Chat inicial
    with st.chat_message("Semaforín", avatar=st.image(avatar_assistant, width=20)):
