...
def main():
    ...
    avatar_assistant = st.image('https://raw.githubusercontent.com/hmdibella/tfm_app/main/perfil_bot.jpg', width=30)
    avatar_user = st.image('https://raw.githubusercontent.com/hmdibella/tfm_app/main/perfil_humano.jpg', width=30)
    for message in st.session_state.messages:
        avatar = avatar_assistant if message["role"] == "Semaforín" else avatar_user
        with st.chat_message(message["role"], avatar=avatar):
            st.markdown(message["content"])
            
    # HDB Chat inicial
    with st.chat_message("Semaforín", avatar=avatar_assistant):
        st.write_stream(response_generator_bot("Hola! 👋 Soy Semaforín 🤖, tu colega-bot que responde preguntas ❓ sobre las leyes de Tráfico y Seguridad Vial 🚗 en España. Hazme las preguntas y yo trataré de responderlas 💪."))
